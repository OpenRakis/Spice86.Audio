...
/// <summary>
/// HSQ decompression for ADG music files packed inside DUNE.DAT.
/// </summary>
/// <remarks>
/// Many files are HSQ-compressed (an LZ77 variant with a 6-byte header).
/// Checksum: <c>(byte0 + byte1 + byte2 + byte3 + byte4 + byte5) &amp; 0xFF == 0xAB</c>.
/// Ported from OpenRakis/tools/dune-ds/source/unhsq.c.
/// </remarks>
public sealed partial class DuneAdgPlayerEngine {
    /// <summary>Maximum decompressed size accepted from an HSQ stream (1 MiB).</summary>
    private const int MaxHsqUncompressedSize = 0x100000;

    /// <summary>
    /// Attempts HSQ decompression; returns <see langword="null"/> if the header
    ...
    private static byte[]? TryDecompressHsq(byte[] source) {
        ...
        if (uncompressedSize == 0 || uncompressedSize > MaxHsqUncompressedSize) {
            return null;
        }
        ...
        try {
            return DecompressHsq(source, uncompressedSize);
        } catch {
            return null;
        }
    }
    ...
    /// <summary>
    /// Core HSQ decompression. Faithful port of <c>unhsq_unpack2</c> from OpenRakis.
    /// </summary>
    ...
    private static bool HsqGetBit(byte[] source, ref int srcPos, ref int bitBuffer) {
        if (bitBuffer == 1) {
            if (srcPos + 1 >= source.Length) { return false; }
